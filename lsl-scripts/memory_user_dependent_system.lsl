...
// 8f846b08-9092-4b5d-a10c-3fbf6f80e509 first prim
// 0fedf54c-dc05-4bb0-8350-c8349a38d793 Georg Schneeberger
list mappings = ["8f846b08-9092-4b5d-a10c-3fbf6f80e509", "llm-system-primus", "0fedf54c-dc05-4bb0-8350-c8349a38d793", "llm-system-dragon"];


// Read Entire Notecard as Single String
string parametersNotecardName = "llm-parameters";
string defaultSystemNotecardName = "llm-system-default";
string userSystemNotecardName = "";

key parametersNotecardQueryId;
key systemNotecardQueryId;
key userSystemNotecardQueryId;
integer parametersCurrentLine = 0;
integer systemCurrentLine = 0;
integer userSystemCurrentLine = 0;
list systemNotecardLines = [];
list userSystemNotecardLines = [];
...
start_oracle(string username) {
    ...
    string instructions = "";
    if (userSystem == ""){
        instructions=defaultSystem;
    } else {
        instructions=userSystem;
    }
    instructions = llReplaceSubString(instructions, "\"", "\\\"", 0);
...
default
{
    state_entry()
    {
        ...
        if (llGetInventoryType(defaultSystemNotecardName) != INVENTORY_NOTECARD)
        {
            llOwnerSay("Error: Notecard '" + defaultSystemNotecardName + "' not found.");
            return;
        }
        ...
        systemNotecardQueryId = llGetNotecardLine(defaultSystemNotecardName, systemCurrentLine);
    ...
    dataserver(key query_id, string data)
    {
        ...
        if (query_id == userSystemNotecardQueryId)
        {
            if (data != EOF)
            {
                ...
                // Add this line to our collection
                userSystemNotecardLines += line;
                
                // Get the next line
                userSystemCurrentLine++;
                userSystemNotecardQueryId = llGetNotecardLine(userSystemNotecardName, userSystemCurrentLine);
            }
            else
            {
                // We've reached the end of the notecard
                // Combine all lines into a single string with newlines
                string entireContent = llDumpList2String(userSystemNotecardLines, "\n");
                
                // Now you have the entire notecard as a single string
                llOwnerSay("Specialized user notecard system loaded.");
                llOwnerSay("Only for you: " + entireContent);

                userSystem = entireContent;
    ...
    touch_start(integer num_detected)
    {
        ...
        if (user!=NULL_KEY & llDetectedKey(0) != user) {
            ...
            integer userIndex = llListFindList(mappings, [user]);
            userSystemNotecardName = "";
            if (userIndex!=-1){
                userSystemNotecardName = llList2String(mappings, userIndex+1);
            }
            if (llGetInventoryType(userSystemNotecardName) == INVENTORY_NOTECARD)
            {
                // llOwnerSay("Will read" + userSystemNotecardName);
                userSystem="";
                userSystemCurrentLine = 0;
                userSystemNotecardLines = [];
                userSystemNotecardQueryId = llGetNotecardLine(userSystemNotecardName, userSystemCurrentLine);
